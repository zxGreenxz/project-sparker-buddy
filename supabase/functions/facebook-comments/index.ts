...
serve(async (req) => {
  ...
  try {
    ...
    if (!pageId || !postId) {
      return new Response(JSON.stringify({ error: "pageId and postId are required" }), {
        status: 400,
    ...
    try {
      ...
      if (comments.length > 0) {
        ...
        // 6. Upsert snapshot
        const { error: upsertError } = await supabaseClient.from("facebook_live_comments_snapshot").upsert(
          {
